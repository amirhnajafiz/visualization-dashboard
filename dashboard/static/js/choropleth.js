function createChoropleth(data, attr) {
  ...
  maxVal = d3.max(data.features, function (d) {
    return +d[attr];
  });
  minVal = d3.min(data.features, function (d) {
    return +d[attr];
  });
  var y = d3.scaleLinear().range([h, 0]).domain([minVal, maxVal]);

  var yAxis = d3.axisRight(y).tickFormat(function (d) {
    if (d >= 100000) return (d / 1000000).toFixed(2) + "M";
    else return d;
  });
  ...
  d3.selectAll(".y-axis text").style("fill", "rgb(155, 155, 155)");
  ...
  function updateChoropleth(data, attr, countries) {
    ...
    g.selectAll("path")
      ...
      .style("fill", function (d) {
        ...
        if (worldmap_country === "world") return ramp(+d[attr]);
    ...
    if (
      ...
      selected_countries.length == maxPCPCountry
    ) {
      ...
      for (i = 0; i < countries_path_arr.length; i++) {
        d3.select(countries_path_arr[i]).style("fill", function (p) {
          ...
          if (
            checkCountry(p.id, selected_countries) ||
            p.id === worldmap_country
          ) {
            return ramp(+p[attr]);
          }
          return "gray";
    ...
    function mouseover(d) {
      if (String(+d[attr]) === "NaN") return;
      if (String(+d[attr]) === "0") return;
    ...
    key.selectAll("g.y-axis").transition().duration(100).call(yAxis);
