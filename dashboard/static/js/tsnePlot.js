...
function createTSNEAnimatedPlot(
  data,
  target,
  features,
  perplexity = 15,
  ...
  const tip = d3
    ...
    .html(function (d) {
      return (
        `<strong>Song Type:</strong> <span class='details'>${
  ...
  function updatePlot() {
    ...
    nodes
      .attr(
        "transform",
        (d, i) => `translate(${positions[i].x},${positions[i].y})`
      )
...
function attachPerplexitySlider(
  sliderId,
  data,
  target,
  features,
